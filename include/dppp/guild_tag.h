...
struct guild_tag {
  ...
  [[nodiscard]] std::string badge_url(image_format fmt = image_format::png,
                                      uint16_t size = 0) const {
    if (badge_hash.empty() || guild_id == 0)
      return {};
    std::string url = std::string(DISCORD_CDN_BASE) + "/guild-tag-icons/" +
                      std::to_string(guild_id) + "/" + badge_hash + "." +
  ...
  [[nodiscard]] static guild_tag
  from_guild_json(const nlohmann::json &guild_json, uint64_t gid = 0) {
    guild_tag gt;
    gt.guild_id = gid;
