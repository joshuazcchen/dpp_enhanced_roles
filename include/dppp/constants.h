...
namespace dppp {
	...
	[[nodiscard]] inline std::string to_hex_color(uint32_t color) { return to_hex_colour(color); }

	[[nodiscard]] inline uint8_t red(uint32_t colour) noexcept { return static_cast<uint8_t>((colour >> 16u) & 0xFFu); }

	[[nodiscard]] inline uint8_t green(uint32_t colour) noexcept {
		return static_cast<uint8_t>((colour >> 8u) & 0xFFu);
	}

	[[nodiscard]] inline uint8_t blue(uint32_t colour) noexcept { return static_cast<uint8_t>(colour & 0xFFu); }
