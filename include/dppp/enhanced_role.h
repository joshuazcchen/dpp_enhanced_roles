...
struct enhanced_role {
  ...
  enhanced_role(const enhanced_role &other)
      : id(other.id), name(other.name), guild_id(other.guild_id),
        position(other.position), colours(other.colours), colors(colours),
        icon_hash(other.icon_hash), unicode_emoji(other.unicode_emoji) {}

  enhanced_role(enhanced_role &&other) noexcept
      : id(std::move(other.id)), name(std::move(other.name)),
        guild_id(std::move(other.guild_id)),
        position(std::move(other.position)), colours(std::move(other.colours)),
        colors(colours), icon_hash(std::move(other.icon_hash)),
  ...
  [[nodiscard]] bool has_unicode_emoji() const noexcept {
    return !unicode_emoji.empty();
  }

  [[nodiscard]] std::string icon_url(image_format fmt = image_format::png,
                                     uint16_t size = 0) const {
    if (icon_hash.empty() || id == 0)
      return {};
    std::string url = std::string(DISCORD_CDN_BASE) + "/role-icons/" +
                      std::to_string(id) + "/" + icon_hash + "." +
  ...
  [[nodiscard]] bool has_enhanced_colour() const noexcept {
    return colours.is_enhanced();
  }
  [[nodiscard]] bool has_enhanced_color() const noexcept {
    return has_enhanced_colour();
  }

  [[nodiscard]] role_style colour_style() const noexcept {
    return colours.style();
  }
  [[nodiscard]] role_style color_style() const noexcept {
    return colour_style();
  }
  ...
  void fill_from_json(const nlohmann::json &j, dpp::snowflake gid = 0) {
    ...
    if (j.contains("id") && !j["id"].is_null()) {
      ...
      if (id_val.is_string()) {
        id =
            static_cast<dpp::snowflake>(std::stoull(id_val.get<std::string>()));
      } else {
  ...
  [[nodiscard]] nlohmann::json colors_to_json() const {
    return colours_to_json();
  }
};
